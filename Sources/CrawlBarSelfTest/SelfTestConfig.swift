...
extension CrawlBarSelfTest {
    ...
    static func testExternalManifestCatalog() throws {
        ...
        try Self.expect(BuiltInCrawlApps.weicrawl.displayName == "WeChat", "weicrawl uses user-facing WeChat name")
        try Self.expect(BuiltInCrawlApps.weicrawl.commands["desktop-cache-import"] == ["--json", "sync", "--source", "desktop-macos", "--keep-source-snapshot"], "weicrawl exposes desktop snapshot import")
        try Self.expect(BuiltInCrawlApps.weicrawl.commands["unlock"] == ["--json", "unlock", "status"], "weicrawl exposes unlock status")
        try Self.expect(BuiltInCrawlApps.weicrawl.capabilities.contains(.desktopCache), "weicrawl advertises desktop cache capability")
        try Self.expect(BuiltInCrawlApps.weicrawl.privacy.containsPrivateMessages, "weicrawl privacy metadata flags WeChat messages")
        try Self.expect(BuiltInCrawlApps.weicrawl.branding.bundleIdentifier == "com.tencent.xinWeChat", "weicrawl uses native WeChat icon")
        try Self.expect(BuiltInCrawlApps.weicrawl.suggestion?.name == "WeChat", "weicrawl suggests from the native WeChat app")
        try Self.expect(BuiltInCrawlApps.weicrawl.install?.package == "vincentkoc/tap/weicrawl", "weicrawl install metadata exists")
